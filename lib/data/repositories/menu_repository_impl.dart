...
class MenuRepositoryImpl implements MenuRepository {
  ...
  MenuRepositoryImpl(this.database);
  ...
  Future<Menu> getMenuByRestaurantId(String restaurantId) async {
    ...
    final categoriesResult = await conn.execute(
      r'''
      SELECT id, name
      FROM categories
      WHERE id IN (
    ...
    for (final row in categoriesResult) {
      ...
      final dishesResult = await conn.execute(
        r'''
      ...
      final dishes = dishesResult.map((dishRow) {
        ...
        final price = priceRaw is String ? double.parse(priceRaw) : (priceRaw as num).toDouble();
